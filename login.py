...
class Login:
    ...
    def register_user(self): # 回傳申請狀態
        try:
            ...
            ########################
            if new_name != '':  # 成功申請帳號
                self.credentials[new_name] = {'username':new_username,
    ...
    def user_login(self, authentication_status):
        # 沒有按申請帳號按鈕的時候才顯示login的警告
        if username is None or username == "" and streamlit.session_state.account is False:
...
if __name__ == "__main__":
    ...
    title_container = streamlit.empty()  # use container to show title

    credentials = login_obj.get_credentials()  # 取得登入資訊
